package com.algo.programmers.prog_dfsBfs_001_타겟_넘버;
...
public class Solution_260304_bfs {
    ...
    private static int bfs(int[] numbers, int target) {
        ...
        int depth = 0;
        int sum = 0;
        q.add(new int[]{depth, sum});

        int result = 0;
        while (!q.isEmpty()) {
            int[] curr = q.poll();
            int currDepth = curr[0];
            int currSum = curr[1];

            if (currDepth == numbers.length) {
                if (currSum == target) result++;
                //System.out.println("@@@ 가지치기 " + "currDepth=" + currDepth + " / currSum=" + currSum + " / result=" + result);
                continue;
            }

            q.add(new int[]{currDepth + 1, currSum + numbers[currDepth]});
            q.add(new int[]{currDepth + 1, currSum - numbers[currDepth]});
        }
