...
public class Solution_260311_dfs {
    ...
    public static String[] solution(String[][] tickets) {
        ...
        list.add(v1);
        int depth = 0;
        dfs(tickets, visited, list, v1, depth);
        return result.toArray(new String[0]);
    }

    private static void dfs(String[][] tickets, boolean[] visited, List<String> list, String v1, int depth) {
        if (!result.isEmpty()) {
            return;
        }
        ...
        for (int i = 0; i < tickets.length; i++) {
            ...
            visited[i] = true;
            list.add(v2);
            dfs(tickets, visited, list, v2, depth + 1);
