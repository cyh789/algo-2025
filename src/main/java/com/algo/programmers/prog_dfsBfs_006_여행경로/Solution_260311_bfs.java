...
public class Solution_260311_bfs {
    ...
    public static String[] solution(String[][] tickets) {
        ...
        return bfs(start, tickets);
    }

    private static String[] bfs(String start, String[][] tickets) {
        List<String> result = new ArrayList<>();
        result.add(start);
        boolean[] visited = new boolean[tickets.length];
        Queue<Node> q = new LinkedList<>();
        q.add(new Node(start, result, visited));
        ...
        while (!q.isEmpty()) {
            ...
            if (currResult.size() == tickets.length + 1) {
                return currResult.stream().toArray(String[]::new);
            }

            for (int i = 0; i < tickets.length; i++) {
                String v2 = tickets[i][1];
                if (!tickets[i][0].equals(v1)) continue;
                ...
                nextResult.add(v2);
                q.add(new Node(v2, nextResult, nextVisited));
            }
        }
    ...
    private static final class Node {
        ...
        private Node(String curr, List<String> result, boolean[] visited) {
            this.curr = curr;
            this.result = result;
        ...
        @Override
        public int hashCode() {
            return Objects.hash(curr, result, visited);
        }
