...
public class Solution_260311_dfs_2 {
    ...
    public static String[] solution(String[][] tickets) {
        ...
        listV2.add(v2);
        int depth = 0;
        dfs(tickets, listV2, visited, v2, depth);
        return result.toArray(new String[0]);
    }

    private static void dfs(String[][] tickets, List<String> listV2, boolean[] visited, String v2, int depth) {
        if (!result.isEmpty()) return;

        if (depth == tickets.length) {
            result = new ArrayList<>(listV2);
            return;
        }
        ...
        for (int i = 0; i < tickets.length; i++) {
            ...
            visited[i] = true;
            listV2.add(nextV2);
            dfs(tickets, listV2, visited, nextV2, depth + 1);
