...
public class Solution_260115_bfs {
    ...
    private static int bfs(int[][] graph, int startX, int startY, int endX, int endY) {
        int[] dx = {-1, 1, 0, 0};
        int[] dy = {0, 0, -1, 1};
        boolean[][] visited = new boolean[101][101];
        Queue<int[]> q = new LinkedList<>();
        int cnt = 0;
        q.add(new int[]{cnt, startX, startY});
        while (!q.isEmpty()) {
            int[] curr = q.poll();
            if (curr[1] == endX && curr[2] == endY) {
                return curr[0] / 2;
            }
            for (int i = 0; i < dx.length; i++) {
                int nextX = curr[1] + dx[i];
                int nextY = curr[2] + dy[i];
                ...
                q.add(new int[]{curr[0] + 1, nextX, nextY});
            }
        }
