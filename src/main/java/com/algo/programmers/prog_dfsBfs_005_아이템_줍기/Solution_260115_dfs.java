...
public class Solution_260115_dfs {
    ...
    public static int solution(int[][] rectangle, int characterX, int characterY, int itemX, int itemY) {
        ...
        cnt = 0;
        dfs(graph, characterX * 2, characterY * 2, itemX * 2, itemY * 2, visited);

        return result == Integer.MAX_VALUE ? 0 : result / 2;
    }

    private static void dfs(int[][] graph, int currX, int currY, int endX, int endY, boolean[][] visited) {
        int[] dx = {-1, 1, 0, 0};
        int[] dy = {0, 0, -1, 1};
...
//            System.out.println("@@@ 가지치기");
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            cnt++;
            dfs(graph, nextX, nextY, endX, endY, visited);
            cnt--;
