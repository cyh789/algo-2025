package com.algo.programmers.prog_1226_dfsBfs_006_여행경로;
...
public class Solution_1_bfs {
    ...
    public static String[] solution(String[][] tickets) {
        ...
        String v2 = "ICN";
        answer.add(v2);
        return bfs(tickets, answer, v2);
    }

    private static String[] bfs(String[][] tickets, List<String> answer, String v2) {
        boolean[] visited = new boolean[tickets.length];
        Queue<Node> q = new LinkedList<>();
        q.add(new Node(v2, answer, visited));

        while (!q.isEmpty()) {
            Node curr = q.poll();
            String currV2 = curr.v2;
            List<String> currAnswer = curr.answer;
            boolean[] currVisited = curr.visited;

            if (currAnswer.size() == tickets.length + 1) {
                return currAnswer.toArray(String[]::new);
            };
            ...
            for (int i = 0; i < tickets.length; i++) {
                ...
                List<String> nextAnswer = new ArrayList<>(currAnswer);
                nextAnswer.add(nextV2);

                boolean[] nextVisited = Arrays.copyOf(currVisited, currVisited.length);
                nextVisited[i] = true;

                q.add(new Node(nextV2, nextAnswer, nextVisited));
            }
        }
    ...
    private static final class Node {
        ...
        private Node(String v2, List<String> answer, boolean[] visited) {
            this.v2 = v2;
            this.answer = answer;
        ...
        @Override
        public int hashCode() {
            return Objects.hash(v2, answer, visited);
        }
