package com.algo.programmers.prog_graph_007_모음사전;

@SuppressWarnings("UnusedAssignment")
public class Solution_260119_dfs {
    ...
    public static int solution(String word) {
        char[] conditions = new char[]{'A', 'E', 'I', 'O', 'U'};
        int depth = 0;
        String curr = "";
        cnt = 0;
        result = 0;
        dfs(word, conditions, depth, curr);
        return result;
    }

    private static void dfs(String word, char[] conditions, int depth, String curr) {
        if (word.equals(curr)) {
            result = cnt;
            return;
        }
        ...
        for (int i = 0; i < conditions.length; i++) {
            ...
            cnt++;
            dfs(word, conditions, depth + 1, next);
            if (result > 0) return;
        }
