package com.algo.programmers.prog_queueNstack_002_기능개발;

import java.util.*;

@SuppressWarnings({"DataFlowIssue", "UnusedAssignment"})
class Solution_260210 {
    ...
    public static int[] solution(int[] progresses, int[] speeds) {
        ...
        for (int i = 0; i < progresses.length; i++) {
            ...
            if (time == 0) {
                ...
                numTemp += speeds[i] * time;

                if (numTemp < 100) {
                    if (resultCnt > 0) {
                        result.add(resultCnt);
                        resultCnt = 0;
                    }
                    while (numTemp < 100) {
                        numTemp += speeds[i];
                ...
                resultCnt++;
                if (i == progresses.length - 1) result.add(resultCnt);
            }
        }
