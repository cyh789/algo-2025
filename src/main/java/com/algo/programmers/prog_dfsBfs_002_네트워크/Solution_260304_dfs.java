package com.algo.programmers.prog_dfsBfs_002_네트워크;

public class Solution_260304_dfs {
    ...
    public static int solution(int n, int[][] computers) {
        ...
        int result = 0;
        for (int v1 = 0; v1 < computers.length; v1++) {
            if (visited[v1]) continue;
            visited[v1] = true;
            dfs(n, computers, visited, v1);
            result++;
        }
    ...
    private static void dfs(int n, int[][] computers, boolean[] visited, int v1) {
        for (int v2 = 0; v2 < n; v2++) {
            if (visited[v2]) continue;
            if (computers[v1][v2] == 0) continue;
            visited[v2] = true;
            dfs(n, computers, visited, v2);
        }
    }
