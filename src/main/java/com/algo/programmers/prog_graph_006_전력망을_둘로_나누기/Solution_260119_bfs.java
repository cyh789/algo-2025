package com.algo.programmers.prog_graph_006_전력망을_둘로_나누기;

import java.util.*;

@SuppressWarnings("UnusedAssignment")
public class Solution_260119_bfs {
    ...
    private static int bfs(List<List<Integer>> graph, int n, Integer v1) {
        ...
        while (!q.isEmpty()) {
            int currV1 = q.poll();
            for (int i = 0; i < graph.get(currV1).size(); i++) {
                int currV2 = graph.get(currV1).get(i);
                if (visited[currV2]) continue;
                visited[currV2] = true;
                cnt++;
                q.add(currV2);
            }
