...
public class Solution_260306_dfs_timeout {
    ...
    public static int solution(int[][] maps) {
        ...
        int cnt = 1;
        dfs(maps, visited, dx, dy, start, end, cnt);

        return result == Integer.MAX_VALUE ? -1 : result;
    }

    private static void dfs(int[][] maps, boolean[][] visited, int[] dx, int[] dy, int[] curr, int[] end, int cnt) {
        if (result <= cnt) {
            //System.out.println("@@@ 가지치기1");
            return;
        }
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            dfs(maps, visited, dx, dy, new int[]{nextx, nexty}, end, cnt + 1);
            visited[nextx][nexty] = false;

            if (result <= cnt) {
                //System.out.println("@@@ 가지치기2");
                return;
            }
        }
    }
