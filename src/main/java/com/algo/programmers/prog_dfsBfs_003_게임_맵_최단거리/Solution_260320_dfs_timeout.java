...
public class Solution_260320_dfs_timeout {
    ...
    public static int solution(int[][] maps) {
        ...
        int cnt = 1;
        dfs(maps, visited, cnt, start, end, dx, dy);
        return result == Integer.MAX_VALUE ? -1 : result;
    }

    private static void dfs(int[][] maps, boolean[][] visited, int cnt, int[] curr, int[] end, int[] dx, int[] dy) {
        if (result <= cnt) {
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            dfs(maps, visited, cnt + 1, new int[]{nextX, nextY}, end, dx, dy);
