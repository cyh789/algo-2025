...
public class Solution_260304 {
    ...
    public static void main(String[] args) {
        ...
        int move = 0;
        dfs(maze, visited, dx, dy, start, end, move);
        System.out.println("출발점으로 복귀! totalMove=" + totalMove);
    }

    private static void dfs(int[][] maze, boolean[][] visited, int[] dx, int[] dy, int[] curr, int[] end, int move) {
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            if (nextX < 0 || nextX > maze.length - 1) continue;
            if (nextY < 0 || nextY > maze[0].length - 1) continue;
            if (maze[nextX][nextY] == 1) continue;
            if (visited[nextX][nextY]) continue;

            visited[nextX][nextY] = true;
            dfs(maze, visited, dx, dy, new int[]{nextX, nextY}, end, move + 1);
            visited[nextX][nextY] = false;
        }

        totalMove++;
        //System.out.println("되돌아감: (" + nextY + "," + nextX + ")" + " / totalMove=" + totalMove);
