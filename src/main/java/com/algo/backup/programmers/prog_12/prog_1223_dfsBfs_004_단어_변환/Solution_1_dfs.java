...
public class Solution_1_dfs {
    ...
    private static void dfs(String begin, String target, String[] words, boolean[] visited, int cnt) {
        if (result <= cnt) {
            //System.out.println("@@@ 가지치기 cnt=" + cnt + " / result=" + result);
            return;
        }
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if(!chkFunc(begin, words[i])) continue;
            visited[i] = true;
    ...
    private static boolean chkFunc(String v1, String v2) {
        char[] v1Temp = v1.toCharArray();
        char[] v2Temp = v2.toCharArray();
        int diff = 0;
        for (int i = 0; i < v1.length(); i++) {
            if (v1Temp[i] != v2Temp[i]) diff++;
        }
