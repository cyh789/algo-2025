package com.algo.backup.programmers.prog_12.prog_1226_dfsBfs_002_네트워크;
...
public class Solution_1_dfs {
    ...
    public static int solution(int n, int[][] computers) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int v1 = i;
            int depth = 0;
            dfs(computers, visited, v1, depth);
            cnt++;
        }
        return cnt;
    }

    private static void dfs(int[][] computers, boolean[] visited, int v1, int depth) {
        visited[v1] = true;
        for (int i = 0; i < computers.length; i++) {
            int v2 = i;
            if (visited[v2]) continue;
            if (computers[v1][v2] != 1) continue;
            dfs(computers, visited, v2, depth + 1);
        }
    }
}