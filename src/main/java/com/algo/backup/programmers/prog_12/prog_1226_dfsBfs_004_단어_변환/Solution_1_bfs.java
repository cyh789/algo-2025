package com.algo.backup.programmers.prog_12.prog_1226_dfsBfs_004_단어_변환;
...
public class Solution_1_bfs {
    ...
    private static int bfs(String begin, String target, String[] words) {
        ...
        q.add(new Node(currStr, cnt, visited));

        while (!q.isEmpty()) {
            Node curr = q.poll();
            if (curr.currStr.equals(target)) return curr.cnt;
            for (int i = 0; i < words.length; i++) {
                if (curr.visited[i]) continue;
                String nextStr = words[i];
                if (!chkFunc(curr.currStr, nextStr)) continue;

                boolean[] nextVisited = Arrays.copyOf(curr.visited, curr.visited.length);
                nextVisited[i] = true;
                q.add(new Node(nextStr, curr.cnt + 1, nextVisited));
            }
        }
    ...
    private static final class Node {
        ...
        private Node(String currStr, int cnt, boolean[] visited) {
        ...
        public boolean equals(Object obj) {
            ...
            var that = (Node) obj;
            return Objects.equals(this.currStr, that.currStr) &&
                    this.cnt == that.cnt &&
        ...
        @Override
        public int hashCode() {
            return Objects.hash(currStr, cnt, visited);
        }
