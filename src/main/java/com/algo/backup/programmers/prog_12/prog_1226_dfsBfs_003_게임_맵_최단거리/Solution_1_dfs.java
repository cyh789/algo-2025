package com.algo.backup.programmers.prog_12.prog_1226_dfsBfs_003_게임_맵_최단거리;
...
public class Solution_1_dfs {
    ...
    public static int solution(int[][] maps) {
        ...
        dfs(maps, dx, dy, visited, cnt, start, end);

        return result == Integer.MAX_VALUE ? -1 : result;
    }

    private static void dfs(int[][] maps, int[] dx, int[] dy, boolean[][] visited, int cnt, int[] curr, int[] end) {
        if (result <= cnt) {
            //System.out.println("@@@ 가지치기 result=" + result + " / cnt=" + cnt);
            return;
        }

        if (curr[0] == end[0] && curr[1] == end[1]) {
            //System.out.println("@@@ 도착 result=" + result + " / cnt=" + cnt);
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            dfs(maps, dx, dy, visited, cnt + 1, new int[]{nextX, nextY}, end);
