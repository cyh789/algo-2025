...
public class Solution_1_dfs {
    ...
    private static void dfs(String curr, String target, String[] words, boolean[] visited, int cnt) {
        if (result <= cnt) return;

        if (curr.equals(target)) {
            //System.out.println("answer=" + result + " / cnt=" + cnt);
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            visited[i] = true;
            //System.out.println(curr + " -> " + next + " / cnt=" + (cnt+1));
            dfs(next, target, words, visited, cnt + 1);
            visited[i] = false;
        }
    }

    private static boolean chkFunc(String curr, String next) {
        char[] currTemp = curr.toCharArray();
        char[] nextTemp = next.toCharArray();
        int diff = 0;
        for (int i = 0; i < currTemp.length; i++) {
            if (currTemp[i] != nextTemp[i]) diff++;
        }
