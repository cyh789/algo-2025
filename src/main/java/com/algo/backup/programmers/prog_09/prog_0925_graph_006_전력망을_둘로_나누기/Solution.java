package com.algo.backup.programmers.prog_09.prog_0925_graph_006_전력망을_둘로_나누기;
...
public class Solution {
    ...
    public static int solution(int n, int[][] wires) {
        ...
        for (int[] v : wires) {
            ...
            int currV1 = v1;
            int depth = 0;
            dfs(graph, currV1, depth, size, visited);

            int cntV2 = n - cntV1;
            int diff = Math.abs(cntV1 - cntV2);
            minCnt = Math.min(minCnt, diff);
    ...
    private static void dfs(ArrayList<ArrayList<Integer>> graph, int currV1, int depth, int size, boolean[] visited) {
        if (depth == size) return;

        cntV1++;
        visited[currV1] = true;

        for (int nextV1 : graph.get(currV1)) {
            if (visited[nextV1]) continue;
            dfs(graph, nextV1, depth + 1, size, visited);
        }
    }
