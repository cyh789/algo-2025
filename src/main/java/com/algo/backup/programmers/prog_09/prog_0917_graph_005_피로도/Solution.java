package com.algo.backup.programmers.prog_09.prog_0917_graph_005_피로도;
...
public class Solution {
    ...
    public static int solution(int k, int[][] dungeons) {
        ...
        int curr = k;
        int exploredCount = 0;
        dfs(dungeons, curr, exploredCount, visited);

        return maxExplored;
    }

    static int maxExplored = 0;
    private static void dfs(int[][] dungeons, int curr, int exploredCount, boolean[] visited) {
        for (int i = 0; i < dungeons.length; i++) {
            ...
            visited[i] = true;
            exploredCount++;
            dfs(dungeons, next, exploredCount, visited);
            exploredCount--;
            visited[i] = false;
        }

        maxExplored = Math.max(maxExplored, exploredCount);
    }
}
