package com.algo.backup.programmers.prog_09.prog_0919_graph_005_피로도;
...
public class Solution {
    ...
    public static int solution(int k, int[][] dungeons) {
        ...
        int curr = k;
        int count = 0;
        dfs(dungeons, curr, count, depth, visited);

        return maxCount;
    }

    static int maxCount = 0;
    private static void dfs(int[][] dungeons, int curr, int count, int depth, boolean[] visited) {
        if (depth == dungeons.length) return;

        maxCount = Math.max(maxCount, count);
        //System.out.println("curr=" + curr + " / count=" + count + " / maxCount=" + maxCount);
        ...
        for (int i = 0; i < dungeons.length; i++) {
            ...
            count++;
            visited[i] = true;
            dfs(dungeons, next, count, i, visited);
            visited[i] = false;
            count--;
        }
