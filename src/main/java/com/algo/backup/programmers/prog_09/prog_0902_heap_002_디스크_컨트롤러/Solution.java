package com.algo.backup.programmers.prog_09.prog_0902_heap_002_디스크_컨트롤러;
...
public class Solution {
    ...
    public static int solution(int[][] jobs) {
        ...
        // 사실상, 작업의 요청 시각이 빠른 것 = 작업의 번호가 작은것
        // 그러면, int[][] jobs -> 작업의 요청 시각으로 sort 오름차순;   int 작업의 요청시각, int 작업의 소요시간
        // PriorityQueue<int[]> pq -> 작업의 소요시간으로 sort 오름차순;   int 작업의 요청시각, int 작업의 소요시간
        // 단, pq는 빈 상태로 시작
        ...
        while (pollCnt < jobs.length) {
            //if (pollCnt >= jobs.length) break;

            //작업 종료시간보다 요청시간이 적은 경우에는 대기큐에 넣음
