package com.algo.backup.programmers.prog_09.prog_0919_graph_006_전력망을_둘로_나누기;
...
public class Solution {
    ...
    public static int solution(int n, int[][] wires) {
        ...
        for (int[] wire : wires) {
            ...
            int curr = v1;
            int cntV1 = dfs(graph, curr, visited);

            int cntV2 = n - cntV1;
            int diff = Math.abs(cntV2 - cntV1);
    ...
    private static int dfs(ArrayList<ArrayList<Integer>> graph, int curr, boolean[] visited) {
        visited[curr] = true;
        int cntV1 = 1;

        for (int next : graph.get(curr)) {
            if (visited[next]) continue;
            cntV1 += dfs(graph, next, visited);
        }
