package com.algo.backup.programmers.prog_09.prog_0901_heap_101_더_맵게;
...
public class Solution {
    ...
    public static int solution(int[] scoville, int K) {
        ...
        //큐에 대기중인 값이 있는경우 반복
        while (!pq.isEmpty()) {
            //가장 작은 값이 스코빌지수를 만족하면, 더 볼 필요 없으므로 종료
            if (pq.peek() >= K) break;

            //1개만 남았으면 더이상 계산이 안되므로 예외처리
            if (pq.size() == 1) {
                //가장 작은 값이 스코빌지수를 만족하면, 더 볼 필요 없으므로 종료
                if (pq.peek() >= K) break;

                //값이 스코빌 지수를 만족하지 못하면, 실패이므로 -1응답
