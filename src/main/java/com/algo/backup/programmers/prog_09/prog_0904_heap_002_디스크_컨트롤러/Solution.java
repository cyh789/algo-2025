package com.algo.backup.programmers.prog_09.prog_0904_heap_002_디스크_컨트롤러;
...
public class Solution {
    ...
    public static int solution(int[][] jobs) {
        ...
        // int[][0] jobs = [int 작업의 요청 시각, int 작업의 소요시간]
        // int[0] jobs = [int 작업의 번호]
        // 사실상 int 작업의 요청 시각 = int 작업의 번호
        Arrays.sort(jobs,(o1, o2) -> o1[0] - o2[0]);    // 오름차순 int 작업의 번호
        PriorityQueue<int[]> pq = new PriorityQueue<>((o1, o2) -> o1[1] - o2[1]);

        int pollCnt = 0;
        int endTime = 0;
        int jobsIdx = 0;    //jobsIdx => 0 ~ jobs.length
        int sum = 0;
        while (pollCnt < jobs.length) {

            //int 작업의 요청 시각이 endTime 보다 적은 모든 경우 체크
            ...
            if (!pq.isEmpty()) {
                ...
                //sum 계산 : endTime (현재시각) + int 작업의 소요시간 - int 작업의 요청 시각
                sum += endTime + selected[1] - selected[0];

                //대기큐에서 꺼냈으므로 endTime += int 작업의 소요시간
                endTime += selected[1];
            } else {
                //endTime 구하기
