package com.algo.backup.programmers.prog_08.prog_0829_queueNstack_004_프로세스_999999999;
...
public class Solution {
    ...
    public static int solution(int[] priorities, int location) {
        ...
        while (!indexQueue.isEmpty()) {
            int prevIdx = indexQueue.poll();
            int prevPriority = priorityMap.get(prevIdx);

            boolean stop = false;
            for (int currIdx : indexQueue) {
                int currPriority = priorityMap.get(currIdx);
            ...
            //큐에 대기중인 프로세스 중 우선순위가 더 높은 프로세스가 있다면 방금 꺼낸 프로세스를 다시 큐에 넣습니다.
            if (stop) {
                indexQueue.offer(prevIdx);
            } else {
                //만약 그런 프로세스가 없다면 방금 꺼낸 프로세스를 실행합니다.
                count++;
                if (location == prevIdx) return count;
            }
        }
