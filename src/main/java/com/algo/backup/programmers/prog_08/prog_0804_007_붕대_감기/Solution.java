package com.algo.backup.programmers.prog_08.prog_0804_007_붕대_감기;

//붕대 감기
public class Solution {
    public static void main(String[] args) {

//        int[] bandage = {5, 1, 5};	int health = 30	;	int[][] attacks = {{2, 10}, {9, 15}, {10, 5}, {11, 5}};
        //answer=5
//        int[] bandage = {3, 2, 7};	int health = 20	;	int[][] attacks = {{1, 15}, {5, 16}, {8, 6}};
//        //answer=-1
//        int[] bandage = {4, 2, 7};	int health = 20	;	int[][] attacks = {{1, 15}, {5, 16}, {8, 6}};
//        //answer=-1
        int[] bandage = {1, 1, 1};	int health = 5	;	int[][] attacks = {{1, 2}, {3, 2}};
//        //answer=3
        int answer = solution(bandage, health, attacks);
        System.out.println(answer);
    }

    public static int solution(int[] bandage, int health, int[][] attacks) {
        //붕대 감기는 t초 동안 붕대를 감으면서
        //  1초마다 x만큼의 체력을 회복합니다.
        //  t초 연속으로 붕대를 감는 데 성공한다면
        //      y만큼의 체력을 추가로 회복합니다.
        //  게임 캐릭터에는 최대 체력이 존재해 현재 체력이 최대 체력보다 커지는 것은 불가능합니다.
        //
        //기술을 쓰는 도중 몬스터에게 공격을 당하면
        //  기술이 취소되고,
        //  공격을 당하는 순간에는 체력을 회복할 수 없습니다.
        //
        //  몬스터에게 공격당해 기술이 취소당하거나 기술이 끝나면
        //      그 즉시 붕대 감기를 다시 사용하며,
        //      연속 성공 시간이 0으로 초기화됩니다.
        //
        //몬스터의 공격을 받으면
        //  정해진 피해량만큼 현재 체력이 줄어듭니다.
        //  이때, 현재 체력이 0 이하가 되면 캐릭터가 죽으며 더 이상 체력을 회복할 수 없습니다.
        //
        //캐릭터가 끝까지 생존할 수 있는지 궁금합니다.
        //
        //붕대 감기 기술의 시전 시간, 1초당 회복량, 추가 회복량을 담은 1차원 정수 배열 bandage
        //최대 체력을 의미하는 정수 health
        //몬스터의 공격 시간과 피해량을 담은 2차원 정수 배열 attacks
        //모든 공격이 끝난 직후 남은 체력을 return 하도록 solution 함수를 완성해 주세요.
        //만약 몬스터의 공격을 받고 캐릭터의 체력이 0 이하가 되어 죽는다면 -1을 return 해주세요.

        int coolTime = bandage[0]; //붕대 감기 기술의 시전 시간
        int healing = bandage[1];  //1초당 회복량
        int bonusHeal = bandage[2]; //추가 회복량

        int currentHp = health;
        System.out.println("currentHp=" + currentHp);
        int time = 1;
        int attackIndex = 0;

        int answer;
        int bonusHealCount = 0;
        while (true) {
            System.out.print("time=" + time);
            if (time == attacks[attackIndex][0]) {
                //몬스터의 공격을 받으면
                //  정해진 피해량만큼 현재 체력이 줄어듭니다.
                //  이때, 현재 체력이 0 이하가 되면 캐릭터가 죽으며 더 이상 체력을 회복할 수 없습니다.
                currentHp -= attacks[attackIndex][1];
                System.out.println(" currentHp=" + currentHp);
                if (currentHp <= 0) {
                    answer = -1;
                    break;
                }
                attackIndex++;

                //기술을 쓰는 도중 몬스터에게 공격을 당하면
                //  기술이 취소되고,
                //  공격을 당하는 순간에는 체력을 회복할 수 없습니다.
                bonusHealCount = 0;
            } else {
                //붕대 감기는 t초 동안 붕대를 감으면서
                //  1초마다 x만큼의 체력을 회복합니다.
                //  t초 연속으로 붕대를 감는 데 성공한다면
                //      y만큼의 체력을 추가로 회복합니다.
                //  게임 캐릭터에는 최대 체력이 존재해 현재 체력이 최대 체력보다 커지는 것은 불가능합니다.
                currentHp += healing;
                bonusHealCount++;
                System.out.println(" currentHp=" + currentHp);
                if (bonusHealCount == coolTime) {
                    currentHp += bonusHeal;
                    bonusHealCount = 0;
                    System.out.println("currentHp + bonusHeal=" + currentHp);
                }
                if (currentHp >= health) {
                    currentHp = health;
                }
            }


            if (time == attacks[attacks.length - 1][0]) {
                answer = currentHp;
                break;
            }

            time++;
        }
        return answer;
    }
}
