package com.algo.backup.programmers.prog_10.prog_1010_dfsBfs_005_아이템_줍기;
...
public class Solution {
    ...
    public static int solution(int[][] rectangle, int characterX, int characterY, int itemX, int itemY) {
        ...
        return bfs(map, characterX * 2, characterY * 2, itemX * 2, itemY * 2, 0) / 2;
    }

    private static int bfs(int[][] map, int x1, int y1, int x2, int y2, int cnt) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (currX == x2 && currY == y2) return currCnt;
    ...
    private static void transMapFunc(int[][] map, int x1, int y1, int x2, int y2) {
        for (int i = x1; i <= x2; i++) {
            for (int j = y1; j <= y2; j++) {
                ...
                map[i][j] = 2;
                if (x1 == i || x2 == i || j == y1 || j == y2) map[i][j] = 1;
            }
        }
    }
