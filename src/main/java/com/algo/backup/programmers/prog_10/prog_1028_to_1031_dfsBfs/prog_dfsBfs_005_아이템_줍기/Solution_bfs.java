...
public class Solution_bfs {
    ...
    public static int solution(int[][] rectangle, int characterX, int characterY, int itemX, int itemY) {
        ...
        for (int[] r : rectangle) {
            ...
            int x2 = r[2] * 2;
            int y2 = r[3] * 2;
            setFunc(temp, x1, y1, x2, y2);
        }
//        for (int i = 0; i < temp.length; i++) {
//            System.out.println(Arrays.toString(temp[i]));
//        }

        return bfs(temp, characterX * 2, characterY * 2, itemX * 2, itemY * 2);
    }

    private static int bfs(int[][] temp, int characterX, int characterY, int itemX, int itemY) {
        int[] dx = {-1, 1, 0, 0};
        int[] dy = {0, 0, -1, 1};
...
//                System.out.println("@@도착 cnt=" + curr[2]);
                cnt = curr[2];
                break;
            }
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nextX < 0 || nextX >= temp.length) continue;
                if (nextY < 0 || nextY >= temp[0].length) continue;
    ...
    private static void setFunc(int[][] temp, int x1, int y1, int x2, int y2) {
        for (int i = x1; i <= x2; i++) {
            for (int j = y1; j <= y2; j++) {
