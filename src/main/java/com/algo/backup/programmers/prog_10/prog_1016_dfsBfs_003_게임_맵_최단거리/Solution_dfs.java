package com.algo.backup.programmers.prog_10.prog_1016_dfsBfs_003_게임_맵_최단거리;
...
public class Solution_dfs {
    ...
    public static int solution(int[][] maps) {
        ...
        int cnt = 1;
        dfs(maps, x, y, visited, cnt);

        return answer == Integer.MAX_VALUE ? -1 : answer;
    }
    static int answer;

    private static void dfs(int[][] maps, int x, int y, boolean[][] visited, int cnt) {
        int[] dx = {-1, 1, 0, 0};
        int[] dy = {0, 0, -1, 1};
        ...
        if (x == maps.length - 1 && y == maps[0].length - 1) {
            if (maps[x][y] == 1) answer = Math.min(answer, cnt);
            return;
        }
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            dfs(maps, nextX, nextY, visited, cnt + 1);
