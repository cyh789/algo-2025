package com.algo.backup.programmers.prog_10.prog_1016_dfsBfs_005_아이템_줍기;
...
//            transMap(map, x1 * 2, y1 * 2, x2 * 2, y2 * 2);
//        }
    ...
    private static int bfs(int[][] map, int characterX, int characterY, int itemX, int itemY) {
        ...
        while (!q.isEmpty()) {
            ...
            if (currX == itemX && currY == itemY) return currCnt / 2;
    ...
    private static void transMap(int[][] map, int x1, int y1, int x2, int y2) {
        for (int i = x1; i <= x2; i++) {
            for (int j = y1; j <= y2; j++) {
