package com.algo.backup.programmers.prog_10.prog_1013_dfsBfs_303_게임_맵_최단거리;
...
public class Solution {
    ...
    private static int bfs(int[][] maps) {
        ...
        int currY = 0;
        int pollCnt = 1;
        queue.add(new int[]{currX, currY, pollCnt});
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < dx.length; i++) {
                if (curr[0] == maps.length - 1 && curr[1] == maps[0].length - 1) return curr[2];

                int nextX = curr[0] + dx[i];
                int nextY = curr[1] + dy[i];
                int nextPollCnt = curr[2];
                ...
                queue.add(new int[]{nextX, nextY, nextPollCnt + 1});
            }
        }
