package com.algo.backup.programmers.prog_11.prog_1121_queueNstack_002_기능개발;
...
class Solution_1 {
    ...
    public static int[] solution(int[] progresses, int[] speeds) {
        ...
        int progressesIdx = 0;
        q.add(progresses[progressesIdx]);
        while (!q.isEmpty()) {
            int curr = q.poll();
            int currSpeed = speeds[progressesIdx];
            ...
            int cnt = 1;
            for (int i = progressesIdx + 1; i < progresses.length; i++) {
                int next = progresses[i];
                int nextSpeed = speeds[i];
                if (next + (nextSpeed * time) >= 100) {
                    cnt++;
                } else {
                    progressesIdx = i;
                    q.add(progresses[progressesIdx]);
